---
layout: home

hero:
    name: "Trific Platform"
    text: "Connecting Clients with Pre-Vetted Service Providers"
    tagline: "A curated marketplace built on trust, quality, and reliability"
    image:
        src: /logo.svg
        alt: Trific Platform
    actions:
        - theme: brand
          text: Get Started
          link: /getting-started/
        - theme: alt
          text: View Platform Overview
          link: /platform/

features:
    - icon: 🚀
      title: Quick Start (5 minutes)
      details: Get up and running with Trific Platform in just 5 minutes with our streamlined setup process.
      link: /getting-started/quick-start
    - icon: 🔒
      title: Enterprise-Grade Security
      details: Built with bank-level security including TLS encryption, OAuth 2.0/OIDC, and comprehensive audit logging.
      link: /platform/security
    - icon: 💳
      title: Secure Payment Processing
      details: Automated escrow management with milestone-based payments and comprehensive fraud protection.
      link: /workflows/payment-escrow-flow/
    - icon: 🛡️
      title: TenderSure Integration
      details: Advanced provider vetting through TenderSure with comprehensive scoring and quarterly re-evaluation.
      link: /workflows/vetting-process/
    - icon: 📊
      title: Role-Based Portals
      details: Specialized interfaces for Clients, Providers, Admins, and Management with tailored workflows.
      link: /portals/
    - icon: 🔗
      title: Developer-Friendly API
      details: Complete REST API with webhooks, SDKs, and comprehensive documentation for custom integrations.
      link: /api/
---

## Choose Your Path

Select the documentation path that matches your role and needs:

::: tip For Clients
**New to Trific?** Start with our [Client Quick Start Guide →](/getting-started/quick-start)  
**Need specific help?** Explore the [Client Portal Documentation →](/portals/client/)  
**Enterprise setup?** Check [Client Journey Workflows →](/workflows/client-journey/)
:::

::: warning For Service Providers
**Getting started?** Begin with [Provider Onboarding →](/workflows/provider-onboarding/)  
**Setting up your profile?** Visit [Provider Portal Guide →](/portals/provider/)  
**Understanding vetting?** Review [TenderSure Vetting Process →](/workflows/vetting-process/)
:::

::: info For Developers & Integrators
**API Integration?** Start with [API Documentation →](/api/)  
**Building custom solutions?** Check [Technical Architecture →](/technical/)  
**Need code examples?** Browse [SDK Documentation →](/api/sdks)
:::

::: note For Administrators
**TenderSure Admin?** Access [Admin Portal Documentation →](/portals/admin/)  
**Trific Management?** Review [Management Portal Guide →](/portals/management/)  
**System operations?** Check [Technical Monitoring →](/technical/monitoring/)
:::

## Platform Highlights

### 🎯 Curated Marketplace

Only pre-vetted, qualified service providers through our rigorous TenderSure evaluation process. **95% client satisfaction rate** with quality assurance built-in.

### 💰 Secure Financial Operations

Automated escrow system with milestone-based payments protecting both clients and providers. **$10M+ processed safely** with comprehensive fraud protection.

### 🔐 Enterprise Security

Bank-level security with TLS 1.3 encryption, OAuth 2.0/OIDC authentication, and comprehensive audit logging. **SOC 2 Type II compliant** infrastructure.

### 📈 Scalable Architecture

Modern microservices architecture with horizontal scaling capabilities. **99.9% uptime** with auto-scaling and robust monitoring.

## Quick Navigation

### 🚀 Getting Started

-   [5-Minute Quick Start](/getting-started/quick-start)
-   [Installation Guide](/getting-started/installation)
-   [First Steps](/getting-started/first-steps)
-   [Troubleshooting](/getting-started/troubleshooting)

### 🏗️ Platform

-   [System Architecture](/platform/architecture)
-   [User Roles & Permissions](/platform/user-roles)
-   [Business Model](/platform/business-model)
-   [Security Framework](/platform/security)

### 🏛️ Portals

-   [Client Portal](/portals/client/)
-   [Provider Portal](/portals/provider/)
-   [Admin Portal](/portals/admin/)
-   [Management Portal](/portals/management/)

### 🔗 API & Integration

-   [API Overview](/api/)
-   [Authentication](/api/authentication)
-   [Webhooks](/api/webhooks/overview)
-   [SDKs & Examples](/api/sdks)

### ⚡ Workflows

-   [Client Journey](/workflows/client-journey/)
-   [Provider Onboarding](/workflows/provider-onboarding/)
-   [Vetting Process](/workflows/vetting-process/)
-   [Payment & Escrow](/workflows/payment-escrow-flow/)

### 🛠️ Resources

-   [Client Handbook](/guides/user-guides/client-handbook)
-   [Provider Handbook](/guides/user-guides/provider-handbook)
-   [FAQ](/support/faq)
-   [Support](/support/troubleshooting)

## Need Help?

Can't find what you're looking for? We're here to help!

### 📚 Documentation

Comprehensive guides and references  
[Browse Documentation →](/getting-started/)

### 💬 Support

Get help from our support team  
[Contact Support →](/support/contact)

### 🐛 Issues

Report bugs or request features  
[GitHub Issues →](https://github.com/TenderSure-Africa-SEZ/trific-platform-docs/issues)

### ❓ FAQ

Common questions and answers  
[View FAQ →](/support/faq)
